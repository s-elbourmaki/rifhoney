    ...
    <section id="collection" className="section-py bg-[#0D0903]">
      <div className="max-w-7xl mx-auto container-px">
        ...
        <div ref={ref} className={`text-center mb-16 transition-all duration-1000 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-[#FDF6E3] text-base md:text-lg lg:text-xl max-w-2xl mx-auto leading-relaxed px-4">
        ...
        {filteredProducts.length > 0 ? (
            ...
            {filteredProducts.map((product, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Product Image & Link */}
                    <Link to={`/product/${product.slug}`} className="relative overflow-hidden aspect-square block">
                      <img
                        src={product.pexels}
                  ...
                  <div className="p-3 sm:p-6 flex flex-col flex-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2 sm:mb-3">
                      {product.tags.map((tag) => (
                        <span key={tag} className="text-[10px] sm:text-[11px] tracking-widest uppercase text-[#C8860A] border border-[#C8860A30] px-1.5 sm:px-2 py-0.5">
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="mb-3 sm:mb-4">
                      <Link to={`/product/${product.slug}`} className="hover:text-[#F5A623] transition-colors">
                        <h3 className="serif text-xl sm:text-2xl font-medium text-[#FDF6E3] leading-tight">{product.name}</h3>
                      </Link>
                      <p className="text-[10px] sm:text-xs tracking-[0.2em] uppercase text-[#C8860A] mt-1 font-medium">{product.subtitle}</p>
                    </div>

                    {/* Weight Selection Chips */}
                    <div className="mb-4 sm:mb-6">
                      <p className="text-[8px] sm:text-[9px] tracking-widest uppercase text-[#FDF6E3] mb-2 sm:mb-3">Select Weight</p>
                      <div className="flex gap-1.5 sm:gap-2">
                        {(['250g', '500g', '1kg'] as Weight[]).map((w) => (
                          <button
                            key={w}
                            onClick={() => setSelectedWeights(prev => ({ ...prev, [product.id]: w }))}
                            className={`flex-1 py-1.5 sm:py-2 text-[8px] sm:text-[10px] tracking-widest uppercase border transition-all duration-300 ${
                              currentWeight === w
                                ? 'border-[#C8860A] text-[#FDF6E3] bg-[#C8860A20]'
                                : 'border-[#C8860A15] text-[#FDF6E3] hover:border-[#C8860A40]'
                            }`}
                          >
                            {w}
                          </button>
                    ...
                    <div className="mt-auto flex flex-col sm:flex-row sm:items-end justify-between gap-3">
                      <div>
                        <div className="flex items-baseline gap-1.5">
                          <span className="serif text-3xl sm:text-4xl font-bold gold-text">{currentPrice}</span>
                          <span className="text-[10px] sm:text-xs text-[#FDF6E3] tracking-wider uppercase font-bold">DH</span>
                        </div>
                      </div>
                      <button
                        onClick={() => handleAdd(product)}
                        className={`relative flex items-center justify-center gap-2 px-4 py-3 sm:px-5 sm:py-3 text-[9px] sm:text-[10px] tracking-[0.2em] uppercase font-bold transition-all duration-400 overflow-hidden active-shrink w-full sm:w-auto ${
                          isAdding
          ...
          <div className="text-center py-20 border border-[#C8860A15] bg-[#150E05]">
            <p className="serif text-2xl text-[#FDF6E3] mb-6">No products found for "{searchTerm}"</p>
            <Link 
              to="/" 
              className="text-[#C8860A] text-xs tracking-widest uppercase border-b border-[#C8860A] pb-1 hover:text-[#F5A623] hover:border-[#F5A623] transition-all"
            >
        ...
        <div className={`text-center mt-16 transition-all duration-1000 delay-500 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <Link 
            to="/catalogue"
            className="inline-block border border-[#C8860A] text-[#F5A623] px-12 py-4 text-xs tracking-[0.3em] uppercase hover:bg-[#C8860A10] transition-all duration-300"
          >
            View Full Catalogue
          </Link>
        </div>
