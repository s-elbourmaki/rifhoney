...
const WHATSAPP_NUMBER = '212622498769';
const WHATSAPP_BASE = 'https://wa.me/';

export function getWhatsAppUrl(message?: string): string {
  const base = `${WHATSAPP_BASE}${WHATSAPP_NUMBER}`;
  if (message) {
...
export default function WhatsAppButton() {
  ...
  // Show after short delay so it doesn't compete with initial page load
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Tooltip label */}
      <span
        className={`hidden sm:block bg-[#1A1208] border border-[#C8860A30] text-[#FDF6E3] text-[11px] tracking-wide px-4 py-2.5 rounded-lg shadow-xl transition-all duration-300 whitespace-nowrap ${
          hovered
